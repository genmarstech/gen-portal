...
| 2026-09-04 | `portal-20260904-094141.dump` | After quotes, permission requests and the widened billing guard. 48 tables | Requests deploy |
| 2026-09-04 | `portal-20260904-090156.dump` + `media-20260904-090156.tar.gz` | After the attachments deploy. 46 tables. **First backup carrying a media archive** — the pair must be restored together, or every attachment row comes back pointing at a file that is not there | Attachments deploy |
| 2026-09-04 | `portal-20260904-082844.dump` | After the client-record deploy. 43 tables, adding `portal_clientprofile`, `portal_hostingarrangement` and `portal_contactlogentry`. Pulled off-box by hand; **not restore-tested** | Client record deploy |
