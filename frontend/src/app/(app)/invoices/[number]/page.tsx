 ...
 *
 * ── WHY IT IS ADDRESSED BY NUMBER AND NOT NESTED UNDER AN ORDER ─────────────
 *
 * It used to live at /dashboard/<reference>/invoices/<number>, which worked
 * only while every invoice had a project behind it. Not every invoice does — a
 * renewal, an afternoon's work, a domain paid for on a past client's behalf —
 * and those have no reference to nest under. They appeared on the invoices
 * list with nothing to open, so a client could be sent a bill their own portal
 * could not show them.
 *
 * An invoice number identifies an invoice on its own. The route now says so.
 *
...
export default function InvoicePage() {
  ...
  /**
   * The saved file is named after the invoice, not after the app.
   *
   * Every browser's Save-as-PDF proposes `document.title` as the filename, so
   * without this a client's accounts folder fills with "Genmars.pdf",
   * "Genmars (1).pdf", "Genmars (2).pdf" — three bills nobody can tell apart
   * without opening them. One line, and the file arrives as
   * "GM-INV-2026-0004.pdf", which is what it will be filed and searched under.
   *
   * Restored on unmount so the title does not follow the reader to the next
   * page they open.
   */
  useEffect(() => {
    if (!number) return;
  ...
  if (error) {
    return (
      <div className="wrap">
        ...
        <p>
          <Link href="/invoices">Back to your invoices</Link>
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="wrap">
      <p className={styles.back}>
        {/* Back to the list, always. The order is a useful second link when
            there is one, but it is not where this page was reached from. */}
        <Link href="/invoices">&larr; Invoices</Link>
        {order ? (
          <>
            {" "}
            <span aria-hidden="true">&middot;</span>{" "}
            <Link href={`/dashboard/${order.reference}`}>{order.title}</Link>
          </>
        ) : null}
      </p>
      ...
      >
            ...
            <tr>
              <td>
                ...
                {order?.contract_reference ? (
                  ...
                  <span className={styles.against}>{order.reference}</span>
                ) : null}
              </td>
            ...
            {payment.stk_available ? (
              <MpesaPay
                ...
                onPaid={() => {
                  ...
                  void portal.invoice(number).then(setDoc);
                }}
              />
            ) : null}
        ...
        <footer className={styles.foot}>
          <p>
            Questions about this invoice go to your point of contact
            {order ? ` on ${order.reference}` : ""}. If our records disagree
            with yours, tell us &mdash; ours is the copy that is wrong until we
            have checked.
          </p>
        </footer>
      </article>
      ...
      <p className={styles.printHint}>
        ...
        {/* Said plainly, because "Download" alone would promise a file and
            deliver a dialog. The dialog is the honest route to a real PDF: it
            is how every browser makes one, and it needs no generator on our
            side (Charter 03 §I — nothing new in the stack without a reason). */}
        <span className={styles.printNote}>
          Choose &ldquo;Save as PDF&rdquo; to keep a copy. It saves as{" "}
          {invoice.number}.pdf
        </span>
      </p>
    </div>
...
function MpesaPay({
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(async () => {
      elapsed += every;
      try {
        const status = await portal.paymentStatus(number);
  ...
  async function submit(event: React.FormEvent) {
    ...
    setNotice(null);
    try {
      const result = await portal.payInvoice(number, phone);
      setNotice(
